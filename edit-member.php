...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $member['member_code'] = trim($_POST['member_code'] ?? $member['member_code']);
    $member['full_name'] = trim($_POST['full_name'] ?? $member['full_name']);
    $member['email'] = trim($_POST['email'] ?? $member['email']);
    $member['phone'] = trim($_POST['phone'] ?? $member['phone']);
    $member['address'] = trim($_POST['address'] ?? $member['address']);
    $member['status'] = trim($_POST['status'] ?? $member['status']);
    ...
    if ($member['member_code'] === '' || $member['full_name'] === '') {
        ...
        $checkStmt = $conn->prepare('SELECT id FROM members WHERE member_code = ? AND id != ? LIMIT 1');
        $checkStmt->bind_param('si', $member['member_code'], $id);
        $checkStmt->execute();
        ...
        if ($checkResult->num_rows > 0) {
            ...
            $updateStmt = $conn->prepare('UPDATE members SET member_code = ?, full_name = ?, email = ?, phone = ?, address = ?, status = ? WHERE id = ?');
            $updateStmt->bind_param('ssssssi', $member['member_code'], $member['full_name'], $member['email'], $member['phone'], $member['address'], $member['status'], $id);
            $updateStmt->execute();
    ...
    <main class="container py-4">
        <section class="card shadow-sm">
            <div class="card-body">
                ...
                <form method="post" action="edit-member.php?id=<?php echo (int) $id; ?>" class="row g-3">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="status" name="status" class="form-select">
                            ...
                            <option value="Inactive" <?php echo ($member['status'] ?? 'Active') === 'Inactive' ? 'selected' : ''; ?>>Inactive</option>
                        </select>
                    </div>
